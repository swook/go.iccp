...
func Stability() {
	...
	var test = func(i int, tcase *testCase) {
		if tcase.encounteredInf {
			return
		}

		newE := tcase.Func(false)
		if math.IsInf(newE, 1) || math.IsInf(newE, -1) {
			tcase.encounteredInf = true
			tcase.List = tcase.List[:i]
			return
		}
		...
		if !tcase.Stopped {
			if math.Abs(newE-tcase.E0) > dElim {
				...
				for x := 0; x < 100; x++ {
					tcase.LineX[x] = logh
					tcase.LineY[x] = logE - (float64(x)-50.0)*barscale
				}
			}
		}
	}
	...
	Plot(
		"Evh",
		fmt.Sprintf("Energy vs Δt with R = %v, gamma = %v (log10-scale)", R, gamma),
		[]string{
			"Δt",
			"Energy / J",
